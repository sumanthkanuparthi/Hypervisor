...
static inline bool vmx_check_support() {
  ...
  //   cprintf("Im in vmx_check_support");
	 /* Your code here */ 
    if(BIT(ecx,5)){
      return true;
	}
...
static inline bool vmx_check_ept() {
   ...
   // cprintf("return false end of the function");
...
struct Page * vmx_init_vmcs() {
    ...
    struct Page *p_vmxon_region = page_alloc( ALLOC_ZERO );
    if(!p_vmxon_region) {
        return NULL;
    }
