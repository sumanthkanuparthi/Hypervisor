...
open(const char *path, int mode)
{
	...
	//cprintf("In open");  
        ...
        struct Fd *fd;
        int r;
	//cprintf("Before fd_alloc");
        if ((r = fd_alloc(&fd)) < 0) {
		 //cprintf("Hello 1:%d",fd);
                return r;
         }
        strcpy(fsipcbuf.open.req_path, path);
        fsipcbuf.open.req_omode = mode;
        //cprintf("before fsipc");
        if ((r = fsipc(FSREQ_OPEN, fd)) < 0) {
                fd_close(fd, 0);
                 //cprintf("Hello 2:%d",fd);

		return r;
        }
        //cprintf("Hello 3:%d",fd); 
        return fd2num(fd);
