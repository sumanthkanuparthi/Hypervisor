...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
    ...
    struct Env *env;
        ...
        if (envid2env(envid, &env, 0) < 0) {
                cprintf("sys_ipc_try_send failed: Bad env %d\n",envid);
                return -E_BAD_ENV;
        }
        ...
        if ((uint64_t)srcva < UTOP){
		  ...
		  if(curenv->env_type == ENV_TYPE_GUEST)
                {
                    pte = epml4e_walk(curenv->env_pml4e, srcva, 0);
                }
		  else
	         {
	    ...
	    envid_t guest, void* guest_pa, int perm)
{
    ...
    // LAB 4: Your code here.
       ...
       struct Env *srcenv, *dstenv;
		...
		return -E_BAD_ENV;
       else if(dstenv->env_type != ENV_TYPE_GUEST)
      {
	  return -E_BAD_ENV;
      }
      else if(((uint64_t)guest_pa+PGSIZE)  > dstenv->env_vmxinfo.phys_sz)
      {
         return -E_INVAL;    
      }
         else if (srcerr == 0 && dsterr==0) {
       	pte_t *pte;
	        ...
	        if ((!(*pte & PTE_W)) && (perm & PTE_W))
                	return -E_INVAL;
                  if (ept_map_hva2gpa(dstenv->env_pml4e, page2kva(pp), guest_pa, perm,1) < 0)
                        return -E_NO_MEM;
		else
			return 0;
	}
